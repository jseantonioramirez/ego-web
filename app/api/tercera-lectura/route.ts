...
export async function POST(req: NextRequest) {
  ...
  // contexto no hay nada que confrontar.
  if (!input || !sesgo || !cuerpo || !preguntaEspejo || !segundaLectura || !preguntaFinal) {
    return NextResponse.json({ error: "Falta el contexto de las lecturas anteriores." }, { status: 400 });
  }
  if (!respuesta2) {
  ...
  if (
    ...
    respuesta.length > MAX_RESPUESTA_LENGTH ||
  ...
  if (!model) {
    ...
    return NextResponse.json(
      { error: "No se pudo generar la tercera lectura. Inténtalo de nuevo en unos minutos." },
      { status: 500 }
    );
  }

  // Una sola llamada sin estado (igual que /api/audit y
  // /api/segunda-lectura): le damos a Claude todo el historial en un
  // único mensaje de usuario, sin reproducir turnos de conversación.
  const contextMessage = [
    ...
    `Pregunta espejo que se le planteó (primera vuelta): "${preguntaEspejo}"`,
    `Respuesta literal del usuario a esa pregunta (primera vuelta): "${respuesta}"`,
    `Segunda lectura ya emitida: "${segundaLectura}"`,
    `Pregunta final que se le planteó (segunda vuelta): "${preguntaFinal}"`,
    `Respuesta literal del usuario a esa pregunta (segunda vuelta): "${respuesta2}"`,
  ]
    .filter(Boolean)
    .join("\n");
  ...
  try {
    ...
    const message = await anthropic.messages.create({
      ...
      tools: [EGO_TERCERA_LECTURA_TOOL],
      tool_choice: { type: "tool", name: "emitir_tercera_lectura" },
    ...
    if (!toolUse) {
      throw new Error("El modelo no devolvió una tercera lectura estructurada.");
    }

    const raw = toolUse.input as Record<string, unknown>;

    const terceraLectura: EgoTerceraLectura = {
      tercera_lectura: String(raw.tercera_lectura ?? ""),
      nota_seguridad:
    ...
    console.error("[EGO /api/tercera-lectura]", err);
    return NextResponse.json(
      { error: "No se pudo generar la tercera lectura. Inténtalo de nuevo en unos minutos." },
      { status: 500 }
    );
